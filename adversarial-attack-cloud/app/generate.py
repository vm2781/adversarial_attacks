...
def classify_batch_with_gemini(images_base64: list, api_key: str) -> list:
    genai.configure(api_key=api_key)
    model = genai.GenerativeModel("gemini-2.0-flash")
    
    content = [
        f"Classify each of the {len(images_base64)} handwritten digits in these images. "
        "Respond with ONLY the digits separated by commas, in order. "
        "Example for 5 images: 3,7,2,9,1"
    ]
    ...
    for attempt in range(3):
        try:
            ...
            digits = response.text.strip().split(",")
            return [int(d.strip()) if d.strip().isdigit() else -1 for d in digits]
...
def run_evaluation(model_name: str, api_key: str, num_examples: int = 20):
    ...
    for i in range(num_examples):
        ...
        predicted = all_predictions[i] if i < len(all_predictions) else -1
